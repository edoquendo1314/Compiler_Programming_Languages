...
final class NonEmptyExpList extends ExpList
{
	...
	/*
	The functions below have a fair amount of code duplication.
	I have opted to keep it for the sake of runtime efficiency, to save runtime overhead for
	conditional branching and/or additional function calls that would be used by more compact code.
	*/

	Val funCallEval(HashMap<String,Val> state, Id func)
    {
        FunVal funVal = (FunVal)func.Eval(state);
        FunDef funDef = funVal.getFunDef();

        ParameterList parameterList = funDef.header.parameterList;

        ExpList currentExpList = this;
        int countp = 0, counte = 0;
        HashMap<String, Val> newMap = new HashMap<String,Val>();

        while (!(parameterList instanceof EmptyParameterList)) {
            if(!(currentExpList instanceof EmptyExpList))
            {
                NonEmptyParameterList nonEmptyParameterList = (NonEmptyParameterList) parameterList;
                NonEmptyExpList nonEmptyExpList = (NonEmptyExpList) currentExpList;

                newMap.put(nonEmptyParameterList.id, nonEmptyExpList.exp.Eval(state));

                parameterList = nonEmptyParameterList.parameterList;
                countp++;
                currentExpList = nonEmptyExpList.expList;
                counte++;
                if(countp != counte)
                {
                    System.out.println("Error: too many arguments for function: " + funDef.header.funName);
                    return null;
                }
            }
            else
            {
                System.out.println("Error: insufficient # of arguments for function: " + funDef.header.funName);
                return null;
            }
        }

        return funDef.exp.Eval(newMap);
    }
